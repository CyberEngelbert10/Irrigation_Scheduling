...
import { Field, FieldUpdateData, REGIONS, CROPS, SOIL_TYPES, IRRIGATION_METHODS, SEASONS } from '@/types/field';
...
export default function EditFieldPage() {
  ...
  const [formData, setFormData] = useState<FieldUpdateData>({
    name: '',
    location: '',
    region: '',
    latitude: undefined,
  ...
  const fetchField = async () => {
    try {
      ...
      setErrors({ general: err.response?.data?.detail || 'Failed to load field' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.current_soil_moisture === undefined || formData.current_soil_moisture < 0 || formData.current_soil_moisture > 100) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await fieldAPI.update(field.id, submitData);
      
      // Success - redirect to field details
      ...
      if (err.response?.data) {
        ...
        setErrors({ general: 'Failed to update field. Please try again.' });
      }
    } finally {
      setSaving(false);
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading field...</p>
        </div>
      </div>
    );
  }

  if (!field || errors.general) {
    return (
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-red-50 border border-red-200 rounded-md p-4">
            <p className="text-sm text-red-800">{errors.general || 'Field not found'}</p>
          </div>
          <div className="mt-4">
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <Link href={`/fields/${field.id}`} className="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 mb-4">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Update the details of {field.name}
          </p>
        </div>
